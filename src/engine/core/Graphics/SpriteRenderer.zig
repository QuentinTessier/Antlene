...
pub const Renderer2D = struct {
    ...
    pub fn flush(self: *Renderer2D) void {
        ...
        Graphics.Rendering.toSwapchain(
            .{
                ...
                .depthLoadOp = if (self.isFirstPassForFrame) .clear else .keep,
                .viewport = .{},
            },
            self,
        ) catch {
        ...
        self.isFirstPassForFrame = false;
    }
    ...
    pub fn drawIsometricSprite(self: *Renderer2D, transform: Transform, region: @Vector(4, f32), texture: ?Graphics.Texture, color: @Vector(4, f32)) void {
        ...
        const position = Math.Mat2x2.mulVec(isoMat, .{ transform.position[0], transform.position[1] }) + @Vector(2, f32){ 0, -16.0 * 16.0 };
        const yOffset = transform.position[2] * 16.0;
        const newTransform = Transform{
            .position = .{ position[0], position[1] + yOffset, 0.0 },
